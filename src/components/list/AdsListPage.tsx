...
import { useAdsStore, type SortMode, type UiCategory } from "../../store/useAdsStore";
...
const AdsListPage = () => {
    ...
    const view = useAdsStore((state) => state.view);
    const sortMode = useAdsStore((state) => state.sortMode);
    const selectedCategories = useAdsStore((state) => state.selectedCategories);
    const onlyNeedsFix = useAdsStore((state) => state.onlyNeedsFix);
    const currentPage = useAdsStore((state) => state.currentPage);
    const searchTerm = useAdsStore((state) => state.searchTerm);

    const setView = useAdsStore((state) => state.setView);
    const setSortMode = useAdsStore((state) => state.setSortMode);
    const toggleCategory = useAdsStore((state) => state.toggleCategory);
    const setOnlyNeedsFix = useAdsStore((state) => state.setOnlyNeedsFix);
    const setCurrentPage = useAdsStore((state) => state.setCurrentPage);
    const setSearchTerm = useAdsStore((state) => state.setSearchTerm);
    const resetFilters = useAdsStore((state) => state.resetFilters);
    ...
    const categoriesKey = useMemo(() => apiCategories.join(","), [apiCategories]);
    ...
    const { data, isLoading, isError, error, refetch } = useQuery({
        ...
        queryFn: async ({ signal }): Promise<ApiItemsResponse> => {
            const params = {
            ...
            return (await getItems({ params, signal })) as ApiItemsResponse;
        },
        staleTime: 60_000,
                    ...
                    <Flex align="center">
                        ...
                        <Menu>
                            ...
                            <Menu.Dropdown>
                                ...
                                <Menu.Item onClick={() => setSortMode("title_asc")}>А → Я</Menu.Item>
                                <Menu.Item onClick={() => setSortMode("title_desc")}>Я → А</Menu.Item>

                                <Menu.Divider />

                                <Menu.Label>Новизна</Menu.Label>
                                <Menu.Item onClick={() => setSortMode("new")}>Сначала новые</Menu.Item>
                                <Menu.Item onClick={() => setSortMode("old")}>Сначала старые</Menu.Item>

                                <Menu.Divider />

                                <Menu.Label>Цена</Menu.Label>
                                <Menu.Item onClick={() => setSortMode("cheap")}>Сначала дешевле</Menu.Item>
